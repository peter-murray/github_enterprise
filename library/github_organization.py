...
def _get_organization_owner(module):
    owner = module.params['owner']
    if owner is None:
        owner = module.params['token_user']
    return owner

def _get_organization_name(module):
    name = module.params['name']

    #TODO we need to replace characters as GHE will do so with respect to characters for users
    name = name.replace('_', '-')
...
def create_organization(module, auth_header):
    ...
    organization_name = _get_organization_name(module)
    base_url = _get_base_url(module)
    owner = _get_organization_owner(module)
